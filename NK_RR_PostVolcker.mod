...
estimation(datafile=Post_Volcker_Data,mode_compute=6,optim=('ncov-mh',500000),cova_compute=1,mode_check,mh_replic=3000000,mh_nblocks=1,mh_drop=0.25,tex) Y R Pi;

// estimation(mh_jscale = 0.02,datafile=NK_Data,mode_compute=0,mode_file=NK_RR_LS_mode,cova_compute=1,mode_check,mh_replic=4000000,mh_nblocks=1,mh_drop=0.25,tex) Y R Pi;

write_latex_prior_table;  

posterior_function(function='Determinacy_Condition',sampling_draws = 5000);
Determinacy_Post=cell2mat(oo_.posterior_function_results(:,1));
[f_Post,xi_Post] = ksdensity(Determinacy_Post);
figure('Name','Determinacy Condition Posterior Distribution')
plot(xi_Post,f_Post);

mean_Post = mean(Determinacy_Post);
save('Determinacy_Cond_Post.mat','f_Post','xi_Post','Determinacy_Post','mean_Post')
